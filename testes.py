...
#de partida encerrada de marca_pontuacao.
    ...
    def test_069_inicia_partida_ok(self):
        print("Caso de Teste 069 - Inicia partida com sucesso.")
    ...
    def test_070_inicia_partida_nok_partida_em_andamento(self):
        print("Caso de Teste 070 - Inicia partida recusa iniciar partida"+
                " com outra partida em andamento.")
        retorno = partida.inicia_partida(["lucas"])
        self.assertEqual( retorno , 2 )

    def test_071_faz_lancamento_nok_indice_invalido(self):
        print("Caso de Teste 071 - Erro em fazer lancamento com indices de "+
                "dados invalidos.")
        retorno = partida.faz_lancamento([-1,5])
        self.assertEqual(retorno, 2)

    def test_072_faz_lancamento_nok_indice_primeiro_lancamento_indice(self):
        print("Caso de Teste 072 - Erro em fazer primeiro lancamento do turno"+
                " com indices de dados escolhidos.")
        retorno = partida.faz_lancamento([1,3])
        self.assertEqual(retorno, 3)

    def test_073_marca_pontuacao_nok_jogador_do_turno_nao_lancou(self):
        print("Caso de Teste 073 - Erro ao marcar pontuacao sem lancar"+
                " dados no turno.")
        retorno = partida.marca_pontuacao('2')
        self.assertEqual( retorno, 3 )

    def test_074_faz_lancamento_ok(self):
        print("Caso de Teste 074 - Faz lancamento em partida com sucesso.")
        retorno = partida.faz_lancamento([])
        self.assertEqual(retorno, 0)

    def test_075_faz_lancamento_ok_reduziu_tentativas(self):
        print("Caso de Teste 075 - Faz lancamento reduz o numero de"+
                " tentativas.")
        info = partida.obtem_info_partida()
        self.assertEqual( info['tentativas'], 2 )

    def test_076_faz_lancamento_nok_tentativas_esgotadas(self):
        print("Caso de Teste 076 - Faz lancamento erro quando o jogador já"+
    ...
    def test_077_faz_lancamento_ok_combinacao_gerada_com_sucesso(self):
        print("Caso de Teste 077 - Faz lancamento combinação gerada.")
        info = partida.obtem_info_partida()
    ...
    def test_078_marca_pontuacao_nok_categoria_invalida(self):
        print("Caso de Teste 078 - Erro ao marcar pontuacao invalida.")
        partida.faz_lancamento([])
        retorno = partida.marca_pontuacao('abacate')
        self.assertEqual( retorno, 2 )

    def test_079_marca_pontuacao_ok_sucesso(self):
        print("Caso de Teste 079 - Marca pontuacao em uma categoria com"+
                " sucesso.")
        partida.obtem_info_partida()['jogador_da_vez']
        retorno = partida.marca_pontuacao('chance')
        self.assertEqual( retorno, 0 )

    def test_080_marca_pontuacao_ok_passou_o_turno(self):
        print("Caso de Teste 080 - Marca pontuacao passou o turno.")
        retorno = partida.obtem_info_partida()['turno']
        self.assertEqual( retorno, 2 )

    def test_081_desiste_ok_condicao_retorno(self):
        print("Caso de Teste 081 - Desiste com sucesso.")
        retorno = partida.desiste('julia')
        self.assertEqual( retorno, 0 )

    def test_082_desiste_ok_retira_jogador(self):
        print("Caso de Teste 082 - Desiste retira jogador da partida.")
        jogadores = partida.obtem_info_partida()['jogadores']
        self.assertNotIn( 'julia', jogadores )

    def test_083_desiste_nok_nome_invalido(self):
        print("Caso de Teste 083 - Erro desiste jogador invalido.")
        retorno = partida.desiste('julia')
        self.assertEqual( retorno, 2 )
...
#################
    ...
    def test_086_pausa_partida_ok(self):
        print("Caso de Teste 086 - Partida pausada com sucesso.")
        data_horario = tabela.obtem_tabelas(['eleanor'],[])[-1]['data_horario']
        retorno = partida.pausa_partida(data_horario) 
        assertEqual( retorno , 0 )

    def test_087_pausa_partida_ok_status_pausada(self):
        print("Caso de Teste 087 - Partida pausada com status 'pausada'.")
        data_horario = tabela.obtem_tabelas(['eleanor'],[])[-1]['data_horario']
        status = partida.obtem_info_partida([data_horario], [])['status']
        assertEqual(status, 'pausada') 

    def test_088_pausa_partida_nok_partida_inexistente(self):
        print("Caso de Teste 088 - Erro ao pausar uma partida inexistente.")
        retorno = partida.pausa_partida(datetime(2000, 1, 11, 12, 21, 41, 0))
        self.assertEqual(retorno, 1)

    def test_089_pausa_partida_nok_partida_pausada(self):
        print("Caso de Teste 089 - Erro ao pausar uma partida pausada.")
        data_horario = tabela.obtem_tabelas(['eleanor'],[])[-1]['data_horario']
        retorno = partida.pausa_partida( data_horario )
        self.assertEqual( retorno, 2 )

    def test_090_pausa_partida_nok_partida_encerrada(self):
        print("Caso de Teste 090 - Erro ao pausar uma partida encerrada.")
